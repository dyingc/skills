...
## The Iron Rule: Narrative lecture, not Q&A review or figure tour

**Your chapter reads like a lecture to someone who has never seen this paper.** Figures, tables, listings, and formulas are *supporting evidence* woven into your narrative explanation. They are NOT the subjects of the explanation.

- **A section heading is a short declarative phrase** — "Driller 的核心循环：fuzzer 与 concolic 的交替协作", "纯 fuzzing 在 magic check 上的失效机制", "Selective concolic execution 的设计取舍"
- **A figure title is NOT a heading** — "Figure 5", "Table I", "Listing 7", "Venn 图" are all nouns. They tell the reader nothing about what they'll learn.
- **A question is NOT a heading** — "为什么单纯 fuzzing 会卡在 magic check 上？" makes the text feel like a quiz. A reader who hasn't read the paper will wonder "why are you asking me this?"

The two most common failure modes are: (1) **one figure = one `####` heading** (museum tour), and (2) **every heading is a question** (Q&A review). Both feel productive to write but leave the reader with no unified understanding.
...
|------|----------------|---------------------|
| **Chapter structure** | `####` headings are short declarative topic phrases; each topic is supported by 0, 1, or several figures woven into ONE narrative; sections flow like a lecture with transitions | One `####` per figure (`#### 概念 3：Venn 图（Figure 5）`); question headings (`#### 为什么 X 会失败？`); disconnected Q&A-style sections |
...
### CORRECT: narrative-first (one `####`, four figures woven into ONE narrative)
...
- **One `####` heading, and it is a declarative topic phrase** ("Driller 的核心循环：fuzzer 与 concolic 的交替协作" — NOT a question).
...
- **Connecting words do the work of turning 4 figures into 1 argument**: "一开始", "一旦识别出卡住", "fuzzer 拿到这条新 seed 之后", "但故事并没有结束". Without these hinges you'd have a photo album; with them you have a lecture.
- **Each figure is followed by ≥150 chars of narrative** that fuses the `level2_breakdown` fields into the topic — `what_to_look_at` becomes "这张图要看的重点是...", `axes_or_structure` becomes "整张 CFG 是自顶向下的...", `key_observations` are integrated into the argument, and `teaching_hook` is the bridge to the NEXT figure.
- **The four fields are NOT sub-headings**. No `**什么要看**` / `**结构**` / `**观察**` / `**教学钩**` bullet list. They are dissolved into flowing prose.
- **The section closes with a summary** ("Driller 的创新点不在于任何一张图各自代表的状态，而在于这四张图连起来...") so the reader leaves with a clear understanding, not a collection of images.

### The judgment test

Before you write any `####` heading, ask yourself:

> **"If a student who has never read this paper sees this heading, will they understand what topic I'm about to teach them?"**

- ✅ `#### Driller 的核心循环：fuzzer 与 concolic 的交替协作` → reader knows "he's going to explain how the loop works"
- ✅ `#### 纯 fuzzing 在 magic check 上的失效机制` → reader knows "he's going to explain why fuzzing gets stuck"
- ✅ `#### Selective concolic execution 的设计取舍` → reader knows "he's going to explain the design tradeoff"
...
- ❌ `#### 为什么单纯 fuzzing 会卡在 magic check 上？` → reader thinks "why is he asking me a question? I don't even know what magic check is yet" (Q&A review mode)
- ❌ `#### LLM 为什么能成为关键突破？` → reader thinks "you haven't explained the problem yet, why are you already asking about solutions?" (assumes prior knowledge)

Every "**he's going to describe X**" heading is a figure-centric failure. Every "**he's asking me a question**" heading is a Q&A-review failure. Only "**he's going to teach me about X**" headings pass.
...
1. [ ] **Before writing anything, I wrote down a list of 3–8 narrative topics this chapter teaches** (declarative phrases, NOT questions, NOT figure names).
2. [ ] **Every `####` heading in my chapter is a short declarative topic phrase** — never a question (ending with `？`), never just a figure / table / listing name.
3. [ ] **grep check**: running `grep -nE '^#{3,4}\s.*(Figure\s*[0-9]|Table\s+[IVXLCDM]|Listing\s*[0-9]|图\s*[0-9]|表\s*[IVXLCDM0-9]|清单\s*[0-9]|？\s*$)' chapters/chapter_XX_output.md` returns **zero** lines.
4. [ ] **No `#### 概念 N：Figure X（...）` pattern**, no `#### 概念 N：Venn 图` / `#### 概念 N：Table I` / `#### 概念 N：Listing 7`, and no question headings like `#### 为什么 X 会失败？`.
5. [ ] **For every topic, I can state in one sentence what the reader should understand after reading it** — not "I described X" but "the reader now understands that X leads to Y".
6. [ ] **When a topic is supported by 2+ figures, those figures live in the SAME `####` section** with connecting narrative ("此时", "然而", "到这里") binding them into one argument. No per-figure sub-headings.
7. [ ] **The chapter has a narrative arc**: opening orientation (2-4 sentences after 前置知识), smooth transitions between sections, and a closing synthesis.
